...
import { heroSlidesFeatureAvailable, listHeroSlideRows } from "@/server/hero-slides";
...
export default async function HeroAdminPage({ searchParams }: PageProps<"/admin/hero">) {
  ...
  const enabled = heroSlidesFeatureAvailable();
  const slides = await listHeroSlideRows();
    ...
    <div className="mx-auto max-w-5xl">
      ...
      {!enabled && (
        <AdminNotice tone="error">
          Le serveur Next actif utilise encore un ancien client Prisma sans le modèle Hero.
          Redémarrer le serveur local ou relancer le déploiement recharge le bon client.
        </AdminNotice>
      )}
